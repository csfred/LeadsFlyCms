...
public class EmailAdminController extends BaseController {
    ...
    /**
     * 邮箱信息设置
     * @param fly_smtp_server
     * @param fly_smtp_port
     * @param fly_smtp_usermail
     * @param fly_smtp_password
     * @return
     */
    @ResponseBody
    @PostMapping(value = "/email_conf_update")
    public DataVo updateEmailConf(
       ...
       try {
            ...
            configService.updateConfigByKey("fly_smtp_server", fly_smtp_server);
            configService.updateConfigByKey("fly_smtp_port", fly_smtp_port);
            configService.updateConfigByKey("fly_smtp_usermail", fly_smtp_usermail);
            configService.updateConfigByKey("fly_smtp_password", fly_smtp_password);
     ...
     * @return
     */
    @GetMapping(value = "/edit_templates_email/{id}")
    public String updateEmailTemplates(@PathVariable(value = "id", required = false) int id,ModelMap modelMap){
        Email email=emailService.findEmailTemplateById(id);
        if(email==null){
     ...
     * @return
     */
    @PostMapping("/email_templates_update")
    @ResponseBody
    public DataVo updateEmailTemplates(@Valid Email email, BindingResult result){
        DataVo data = DataVo.failure("操作失败");
        try {
            if (result.hasErrors()) {
            ...
            data = emailService.updateEmailTemplatesById(email);
        } catch (Exception e) {
